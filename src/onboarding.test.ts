...
test("one short brief personalizes a private workspace without a résumé and preserves edits", () => {
  ...
  try {
    ...
    writeFileSync(join(root,"profile/CONTENT.md"),brief.replace("Tone:","Model: grok\nTone:"));
    writeFileSync(join(root,".env"),'XAI_API_KEY="fixture-root-model-key"\nAI_CONTENT_MODEL_API_KEY="wrong-provider-key"\nAI_CONTENT_MODEL_PROVIDER="zai"\nGOOGLE_CLIENT_SECRET="must-stay-out"\n');
    configureExecutive(root);
    const envFile=join(first.root,".env");
    assert.deepEqual(parseEnv(readFileSync(envFile,"utf8")),{XAI_API_KEY:"fixture-root-model-key"});
    ...
    writeFileSync(join(root,"profile/.env"),'XAI_API_KEY="fixture-updated-model-key"\nGOOGLE_CLIENT_SECRET="must-stay-out"\n');
    assert.equal(configureExecutive(root).changed,false);
    assert.deepEqual(parseEnv(readFileSync(envFile,"utf8")),{XAI_API_KEY:"fixture-updated-model-key",ZAI_API_KEY:"old-generic-key",VIDEO_PATH:"C:\\Users\\Jordan\\Videos"});
